...
begin
  ...
  where si.status='completed' and not exists(select 1 from public.test_results tr where tr.schedule_item_id=si.id)
    and not exists(select 1 from public.exam_attendance_records ear join public.exam_attendance_sessions eas on eas.id=ear.exam_attendance_session_id where eas.schedule_item_id=si.id and eas.status='submitted' and ear.status='absent')
  on conflict (type,reference_id) where status='open' do nothing;
  ...
  where ass.recorded_days>=5 and ass.attendance_percentage<75
  on conflict (type,reference_id) where status='open' do nothing;
  ...
  from (
    select ats.class_id,round(100.0*count(*) filter (where ar.status in ('present','late','excused'))/nullif(count(*),0),1) attendance_percentage
    from public.attendance_records ar join public.attendance_sessions ats on ats.id=ar.session_id
    where ats.attendance_date=current_date and ats.status='submitted'
    group by ats.class_id
  ) x where x.attendance_percentage<85
  on conflict (type,reference_id) where status='open' do nothing;
  ...
  where a.status='open' and a.type='results_missing'
    and exists(select 1 from public.test_results tr where tr.schedule_item_id=a.reference_id);
