...
begin
  ...
  insert into public.alerts(type,severity,teacher_id,reference_table,reference_id,message)
  select 'teacher_compliance_warning','high',p.user_id,'profiles',p.user_id,format('%s has multiple recent paper-compliance warnings.',p.full_name)
  from public.profiles p
  where p.role='teacher' and p.is_active and (select count(*) from public.alerts a where a.type='paper_missing' and a.teacher_id=p.user_id and a.created_at>=now()-interval '30 days')>=2
  on conflict (type,reference_id) where status='open' do nothing;
  ...
  where x.pass_rate<40
  on conflict (type,reference_id) where status='open' do nothing;

  insert into public.alerts(type,severity,recipient_id,reference_table,reference_id,message)
  select 'student_performance_warning','critical',p.user_id,'students',x.student_id,format('Student %s has failed three consecutive graded tests.',x.student_name)
  ...
  update public.alerts a set status='resolved',resolved_at=now() and exists(select 1 from public.result_submissions rs where rs.schedule_item_id=a.reference_id and rs.status in ('reviewed','finalized'));
