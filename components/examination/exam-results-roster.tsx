...
function ExamResultsRosterInner({scheduleItemId,students,existingResults}:ExamResultsRosterProps){
 ...
 return <div className="flex flex-col gap-3"><div className="grid gap-3 sm:grid-cols-[160px_minmax(0,1fr)]"><Input label="Total marks" type="number" min={1} value={totalMarks} onChange={e=>setTotalMarks(e.target.value)}/><Input label="Search students" type="search" placeholder="Roll number or name..." value={search} onChange={e=>setSearch(e.target.value)}/></div><ul className="flex flex-col gap-2">{visible.map(student=>{const row=rows[student.id];return <li key={student.id} className="flex flex-col gap-2 rounded-xl border border-neutral-200 p-3"><div className="flex items-center justify-between gap-2"><div className="min-w-0"><p className="truncate text-sm font-medium text-neutral-900">{student.name}</p><p className="text-xs text-neutral-500">Roll #{student.roll_no}</p></div>{row.isPass!==null&&!row.absent&&<Badge variant={row.isPass?"success":"danger"}>{row.isPass?"Pass":"Fail"}</Badge>}</div><div className="flex flex-wrap items-center gap-3"><div className="w-28"><Input label="Marks" type="number" min={0} disabled={row.absent} value={row.marks} onChange={e=>update(student.id,{marks:e.target.value})}/></div><label className="flex items-center gap-2 pt-5 text-sm text-neutral-700"><input type="checkbox" className="h-4 w-4 rounded border-neutral-300 text-primary-600" checked={row.absent} onChange={e=>update(student.id,{absent:e.target.checked,marks:e.target.checked?"":row.marks})}/>Absent</label></div></li>})}</ul><div className="flex flex-wrap gap-2"><Button type="button" variant="secondary" loading={saving} onClick={save}>Save all</Button><Button type="button" variant="primary" loading={submitting} onClick={submit}>Submit results for review</Button></div></div>;
}
