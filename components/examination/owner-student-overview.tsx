...
export function OwnerStudentOverview({ classes, sections, students }: OwnerStudentOverviewProps) {
  ...
  const largest = classRows.reduce<(typeof classRows)[number] | null>(
    (current, row) => (!current || row.count > current.count ? row : current),
    null
  );
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-neutral-500">
              Executive visibility into enrolment, class distribution and roster health. Routine record editing stays with the Clerk.
            </p>
          </div>
          <Link href="/owner/students/directory" className="inline-flex items-center justify-center rounded-xl bg-neutral-950 px-4 py-2.5 text-sm font-medium text-white hover:bg-neutral-800">
            Open directory
          </Link>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-neutral-200 bg-white shadow-sm">
        <div className="border-b border-neutral-100 px-5 py-4 sm:px-6">
          <div className="flex items-center justify-between gap-3">
            ...
            <span className="rounded-full bg-neutral-100 px-2.5 py-1 text-xs font-medium text-neutral-600">Live roster</span>
          </div>
        </div>
          ...
          {classRows.length === 0 ? <p className="px-5 py-8 text-sm text-neutral-500 sm:px-6">No classes found.</p> : classRows.map(({ klass, count, sectionCount }) => (
            <div key={klass.id} className="flex items-center justify-between gap-4 px-5 py-4 sm:px-6">
              <div className="min-w-0">
                <p className="font-medium text-neutral-900">{klass.name}</p>
                <p className="mt-0.5 text-xs text-neutral-500">{sectionCount} {sectionCount === 1 ? "section" : "sections"}{klass.group_name ? ` · ${klass.group_name}` : ""}</p>
              </div>
      ...
      <section className="grid gap-3 lg:grid-cols-2">
        ...
        <Insight title="Owner workflow" value="Observe → escalate" text="Use the directory for visibility; send record corrections and routine roster work to the Clerk." />
      </section>
    </div>
  );
}
