...
function ExamPaperEditorInner({ scheduleItemId, status, initialContent, initialFilePath }: ExamPaperEditorProps) {
  ...
  const handleSubmit = async () => {
    ...
    if (file) {
      ...
      if (result.error) return toast(result.error, "danger"); toast("Paper uploaded and submitted. Clerk and coordinator can now see it.", "success"); if (fileRef.current) fileRef.current.value = ""; router.refresh(); return;
    }
  ...
  return <div className="flex flex-col gap-3">
    ...
    {canEdit ? <>
      ...
      <div className="flex flex-wrap gap-2"><Button type="button" variant="secondary" loading={savingDraft} disabled={submitting} onClick={handleSaveDraft}>Save draft</Button><Button type="button" variant="primary" loading={submitting} disabled={savingDraft || (!content.trim() && !fileRef.current?.files?.length)}>Submit paper</Button></div>
