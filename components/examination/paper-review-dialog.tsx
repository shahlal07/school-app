...
// A teacher may legitimately submit a paper as text content only (the
// editor's own label says "optional when uploading a file") - "file_present"
// only makes sense, and is only required for approval, when the paper
// actually has a file_path. Otherwise this checklist item doesn't apply and
// must not block an otherwise-complete text-only submission from ever being
// approvable.
...
export function PaperReviewDialog({row,onClose}:PaperReviewDialogProps){
 ...
 const hasFile=Boolean(row?.paper?.file_path);
 const requiredChecks=checks.filter(([k])=>k!=="file_present"||hasFile);
 const handleApprove=async()=>{if(!row?.paper)return;const all=requiredChecks.every(([k])=>quality[k]);if(!all)return toast("Complete all quality checks before approving.","danger");setSubmittingAction("approve");const result=await approvePaper(row.paper.id,notes,{...quality,file_present:hasFile});setSubmittingAction(null);if(result.error)return toast(result.error,"danger");toast("Paper approved","success");setNotes("");onClose();router.refresh();};
 ...
 return <Dialog open={row!==null} onClose={handleClose} title={row?.scheduleItem.title??undefined} description={row?`${row.className} - ${row.subjectName}`:undefined}>{row&&<div className="flex flex-col gap-4">
  ...
  <div><div className="flex items-center justify-between"><span className="text-sm font-medium text-neutral-700">Paper quality checklist</span><span className="text-xs text-neutral-500">{requiredChecks.filter(([k])=>quality[k]).length}/{requiredChecks.length}</span></div><div className="mt-2 grid gap-2 sm:grid-cols-2">{requiredChecks.map(([key,label])=><label key={key} className="flex items-center gap-2 rounded-lg bg-neutral-50 p-2 text-sm text-neutral-700"><input type="checkbox" checked={quality[key]} onChange={e=>setQuality(prev=>({...prev,[key]:e.target.checked}))} disabled={submitting}/>{label}</label>)}</div>{!hasFile&&<p className="mt-1.5 text-xs text-neutral-400">No file was uploaded - this paper was submitted as text content only, so &quot;Paper file attached&quot; isn&apos;t required.</p>}</div>
