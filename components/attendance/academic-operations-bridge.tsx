    ...
    <div className="mt-6">
      <section className="rounded-2xl border border-neutral-200 bg-white shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-3 p-4 sm:grid-cols-4 sm:p-5">
          ...
          <div className="rounded-xl bg-neutral-50 p-3"><p className="text-xs text-neutral-500">Attendance trend</p><p className="mt-1 text-2xl font-semibold tabular-nums text-neutral-900">{trend.length}</p><p className="mt-1 text-xs text-neutral-500">days with submitted roll</p></div>
        </div>
              ...
              {signals.slice(0, 8).map((student) => (
                <div key={student.student_id} className="rounded-xl border border-neutral-100 px-3 py-3">
                  <div className="flex items-start gap-3">
                    <span className="w-10 shrink-0 pt-0.5 text-center text-sm font-semibold tabular-nums text-neutral-500">{student.roll_no}</span>
                    <div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold text-neutral-900">{student.name}</p><p className="mt-0.5 text-xs text-neutral-500">Attendance {student.attendance_percentage ?? "—"}% · Assessment {student.assessment_percentage ?? "—"}%</p></div>
                    <span className={`shrink-0 rounded-full px-2 py-1 text-[11px] font-semibold ${signalClass(student.signal)}`}>{signalLabel(student.signal)}</span>
                  </div>
                </div>
              ))}
