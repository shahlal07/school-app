...
export default async function CoordinatorResultsPage(){
  ...
  return <main className="flex flex-col gap-5 p-4 sm:p-6"><div><h1 className="text-xl font-semibold text-neutral-900">Results pipeline</h1><p className="mt-1 text-sm text-neutral-500">Draft → teacher submitted → coordinator reviewed → finalized. Finalized rows are locked in the database.</p></div>
    ...
    <Card><CardHeader><CardTitle>Submission queue</CardTitle></CardHeader><CardContent>{submissions.length===0?<p className="text-sm text-neutral-500">No result submissions yet. Teachers can submit a completed roster from each exam.</p>:<div className="flex flex-col gap-2">{submissions.map(s=>{const item=scheduleMap.get(s.schedule_item_id);return <div key={s.id} className="flex flex-col gap-2 rounded-xl border border-neutral-200 p-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-medium text-neutral-900">{item?.title??'Unknown exam'}</p><p className="text-xs text-neutral-500">{teacherMap.get(s.teacher_id)?.full_name??'Unknown teacher'} · {item?.scheduled_date??''} · {s.status}</p>{s.review_notes&&<p className="mt-1 text-xs text-neutral-600">{s.review_notes}</p>}</div>{canManage&&s.status==='submitted'&&<div className="flex gap-2"><form action={async()=>{'use server';await finalizeResult(s.id)}}><button className="rounded-lg bg-primary-600 px-3 py-1.5 text-xs font-medium text-white">Finalize</button></form><form action={async()=>{'use server';await rejectResult(s.id,'Please correct and resubmit the result roster.')}}><button className="rounded-lg bg-neutral-100 px-3 py-1.5 text-xs font-medium text-neutral-900">Reject</button></form></div>}</div>})}</div>}</CardContent></Card>
  </main>;
}
