...
import { createIntervention, updateIntervention } from "./actions";
...
export default async function CoordinatorInterventionsPage(){
  ...
  const [studentsRes,subjectsRes,teachersRes,interventionsRes]=await Promise.all([
    ...
    supabase.from("academic_interventions").select("id,student_id,subject_id,assigned_to,action,notes,due_date,status,outcome,created_at").order("created_at",{ascending:false})
  ]);
  ...
  const interventions=(interventionsRes.data as {id:string;student_id:string;subject_id:string|null;assigned_to:string|null;action:string;notes:string|null;due_date:string|null;status:string;outcome:string|null;created_at:string}[]|null)??[];
  const studentMap=new Map(students.map(s=>[s.id,s])); const subjectMap=new Map(subjects.map(s=>[s.id,s])); const teacherMap=new Map(teachers.map(t=>[t.user_id,t]));
  return <main className="flex flex-col gap-5 p-4 sm:p-6">
    ...
    <Card><CardHeader><CardTitle>Create intervention</CardTitle></CardHeader><CardContent>
      <form action={createIntervention} className="grid gap-3 sm:grid-cols-2">
        ...
        <label className="text-sm sm:col-span-2">Action<input name="action" required placeholder="e.g. 3 targeted revision sessions on algebra" className="mt-1 w-full rounded-xl border border-neutral-200 p-2"/></label>
    ...
    <Card><CardHeader><CardTitle>Active and completed interventions</CardTitle></CardHeader><CardContent>
      {interventions.length===0?<p className="text-sm text-neutral-500">No interventions have been recorded yet.</p>:<div className="flex flex-col gap-3">{interventions.map(i=><div key={i.id} className="rounded-xl border border-neutral-200 p-3"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-sm font-semibold text-neutral-900">{studentMap.get(i.student_id)?.name??"Unknown student"}</p><p className="text-xs text-neutral-500">{subjectMap.get(i.subject_id??"")?.name??"General"} · {teacherMap.get(i.assigned_to??"")?.full_name??"Unassigned"}{i.due_date?` · due ${i.due_date}`:""}</p></div><span className="rounded-full bg-neutral-100 px-2 py-1 text-xs font-medium">{i.status}</span></div><p className="mt-2 text-sm text-neutral-700">{i.action}</p>{i.notes&&<p className="mt-1 text-xs text-neutral-500">{i.notes}</p>}<form action={async()=>{"use server";await updateIntervention(i.id,i.status==='completed'?'open':'completed',i.outcome??'')} className="mt-2"><button type="submit" className="text-xs font-medium text-primary-600">{i.status==='completed'?'Re-open':'Mark completed'}</button></form></div>)}</div>}
    </CardContent></Card>
  </main>;
}
