...
export async function createIntervention(formData:FormData):Promise<Result>{
  ...
  if(!studentId||!action)return{error:t("coordinator.interventions.validationRequired")};
...
export async function updateIntervention(id:string,status:string,outcome:string):Promise<Result>{
  const profile=await requireRole("academic_coordinator"); const supabase=createClient(); const t=await getT();
  if(!["open","in_progress","completed","cancelled"].includes(status))return{error:t("coordinator.interventions.invalidStatus")};
