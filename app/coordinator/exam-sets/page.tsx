  ...
  if (classesWithSubjects.length === 0) {
    return (
      <main className="flex flex-col gap-5 p-4 sm:p-6">
        <div>
          <h1 className="text-xl font-semibold text-neutral-900">{t("coordinator.examSets.title")}</h1>
          <p className="mt-1 text-sm text-neutral-500">
            {t("coordinator.examSets.emptySubtitle")}
          </p>
        </div>
        <Card>
          <CardContent>
            <EmptyState
              title={t("coordinator.examSets.noClassesTitle")}
              description={t("coordinator.examSets.noClassesDescription")}
            />
          </CardContent>
        </Card>
      </main>
  ...
  return (
    <main className="flex flex-col gap-5 p-4 sm:p-6">
      <div>
        <h1 className="text-xl font-semibold text-neutral-900">{t("coordinator.examSets.title")}</h1>
        <p className="mt-1 text-sm text-neutral-500">
          {t("coordinator.examSets.subtitle")}
        </p>
      </div>
            ...
            <Card key={klass.id}>
              <CardHeader>
                ...
                <p className="mt-1 text-xs text-neutral-500">
                  {activeSubjects.length} {activeSubjects.length === 1 ? t("coordinator.examSets.activeSubjectSingular") : t("coordinator.examSets.activeSubjectPlural")}
                </p>
              </CardHeader>
              <CardContent className="flex flex-col gap-3">
                {!latestSet ? (
                  <p className="text-sm text-neutral-500">{t("coordinator.examSets.noSetStarted")}</p>
                ) : (
                  <div className="rounded-xl border border-neutral-200 p-3">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      ...
                      >
                        {t("coordinator.examSets.setWord")} #{latestSet.set_number} · {t("coordinator.examSets.viewReport")}
                      </Link>
                            ...
                            return (
                              ...
                              >
                                <span className="text-neutral-700">
                                  <Bdi>{subject?.name ?? t("coordinator.fallback.unknownSubject")}</Bdi>
                                  {row.scheduled_date ? <> · <Bdi>{row.scheduled_date}</Bdi></> : ""}
                                </span>
