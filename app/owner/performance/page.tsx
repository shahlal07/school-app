...
export default async function PerformancePage() {
  ...
  // Nothing has been graded yet (true right now - this is a brand-new school
  // year and zero test_results rows exist). Rather than render zeroed-out
  // stat cards or an empty chart shell, be honest that there's nothing to
  // show yet. The moment teachers start entering results, the branch below
  // takes over and renders real aggregates.
  ...
  for (const result of gradedResults) {
    ...
    if (!scheduleItem) continue; // orphaned result row - skip rather than guess
  ...
  for (const [topicId, acc] of Array.from(topicAcc.entries())) {
    if (acc.total < MIN_TOPIC_SAMPLE_SIZE) continue; // avoid flagging off a single data point
