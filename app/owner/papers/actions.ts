...
async function markLatestVersion(
  ...
  const { data: version } = await supabase
    ...
    .limit(1)
    .maybeSingle();
  if (version?.id) await supabase.from("exam_paper_versions").update({ status }).eq("id", version.id);
}
...
export async function approvePaper(paperId: string, reviewNotes: string): Promise<ActionResult> {
  ...
  const { error } = await supabase
    .from("exam_papers")
    .update({ status: "approved", reviewed_at: new Date().toISOString(), reviewed_by: profile.user_id, review_notes: reviewNotes.trim() || null })
    .eq("id", paperId);
  if (error) return { error: error.message };
  await markLatestVersion(supabase, paperId, "approved");
  await supabase.from("alerts").insert({
    type: "paper_approved", severity: "info", teacher_id: paper.teacher_id, recipient_id: paper.teacher_id,
    reference_table: "exam_papers", reference_id: paperId,
    message: "Your exam paper was approved and is ready for the clerk to place for printing."
  });
...
export async function rejectPaper(paperId: string, reviewNotes: string): Promise<ActionResult> {
  ...
  const { error } = await supabase
    .from("exam_papers")
    .update({ status: "draft", reviewed_at: new Date().toISOString(), reviewed_by: profile.user_id, review_notes: trimmedNotes })
    .eq("id", paperId);
  if (error) return { error: error.message };
  await markLatestVersion(supabase, paperId, "rejected");
  await supabase.from("alerts").insert({
    type: "paper_rejected", severity: "warning", teacher_id: paper.teacher_id, recipient_id: paper.teacher_id,
    reference_table: "exam_papers", reference_id: paperId,
    message: `Your exam paper needs changes: ${trimmedNotes}`
  });
