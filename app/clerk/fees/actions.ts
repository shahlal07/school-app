...
export async function createFeeRecord(formData: FormData): Promise<void> {
  const profile = await requireRole("clerk");
  const supabase = createClient();
  const studentId = String(formData.get("student_id") ?? "");
  const amount = Number(formData.get("amount"));
  const dueDate = String(formData.get("due_date") ?? "");
  if (!studentId || !Number.isFinite(amount) || amount < 0 || !dueDate) throw new Error("Student, amount and due date are required.");
...
export async function updateFeeStatus(formData: FormData): Promise<void> {
  const profile = await requireRole("clerk");
  const id = String(formData.get("id") ?? "");
  const status = String(formData.get("status") ?? "");
  if (!["pending", "paid", "overdue"].includes(status)) throw new Error("Invalid status.");
