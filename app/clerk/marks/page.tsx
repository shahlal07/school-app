...
export default async function ClerkMarksPage({ searchParams }: { searchParams?: { id?: string } }) {
  ...
  const past = (schedules ?? []).filter(s => s.scheduled_date <= today && !["cancelled","skipped"].includes(s.status));
  if (!selectedId) {
    const teacherIds=Array.from(new Set(past.map(s=>s.teacher_id).filter(Boolean)));
    const [{data:classes},{data:subjects},{data:teachers}] = await Promise.all([
      supabase.from("classes").select("id,name"),supabase.from("subjects").select("id,name"),teacherIds.length ? supabase.from("profiles").select("user_id,full_name").in("user_id",teacherIds) : Promise.resolve({data:[] as {user_id:string;full_name:string}[]})
    ]);
  ...
  const [{data:teacher},{data:students},{data:results}]=await Promise.all([
    schedule.teacher_id ? supabase.from("profiles").select("user_id,full_name").eq("user_id",schedule.teacher_id).maybeSingle() : Promise.resolve({data:null}),
