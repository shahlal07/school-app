...
/**
 * Direct mirror of app/owner/results/page.tsx - that page was already
 * fully read-only (a list of links into /teacher/exams/<id>, no mutation
 * UI at all), so no readOnly gating was needed here; the query pattern and
 * markup are reused verbatim.
 */
  ...
  return (
    <main className="p-4 sm:p-6">
      ...
      <p className="mt-1 text-sm text-neutral-500">
        {t("principal.results.subtitle")}
      </p>

      <div className="mt-5">
        {completedItems.length === 0 ? (
          <EmptyState
            title={t("principal.results.emptyTitle")}
            description={t("principal.results.emptyDescription")}
          />
        ) : (
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-neutral-500">
                        <Bdi>{subjectById.get(item.subject_id)?.name ?? t("principal.common.unknownSubject")}</Bdi> ·{" "}
                        <Bdi>{classById.get(item.class_id)?.name ?? t("principal.common.unknownClass")}</Bdi> ·{" "}
                        <Bdi>{formatDate(item.scheduled_date)}</Bdi>
                      </p>
                    </div>
                    <Badge variant={complete ? "success" : "warning"}>
                      <Bdi>{graded}</Bdi>/<Bdi>{totalStudents || "?"}</Bdi> {t("principal.results.graded")}
                    </Badge>
                  </Link>
                </li>
              );
