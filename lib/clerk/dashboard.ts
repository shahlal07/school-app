...
export async function getClerkOperationalSummary() {
  ...
  const now = new Date();
  const monthStart = new Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Karachi", year: "numeric", month: "2-digit", day: "2-digit" }).format(new Date(now.getFullYear(), now.getMonth(), 1));
  const [documents, admissions, fees, recentDocuments, recentAdmissions, recentFees] = await Promise.all([
    ...
    supabase.from("admissions").select("id", { count: "exact", head: true }).in("status", ["inquiry", "pending"]).gte("applied_at", `${monthStart}T00:00:00+05:00`),
  ...
  return {
    ...
    tasks: {
      ...
      admissions: recentAdmissions.data ?? [],
