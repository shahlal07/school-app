...
export async function getAcademicIntelligenceData():Promise<AcademicIntelligenceData>{
  ...
  const readiness=schedules
    ...
    .map(s=>{
      ...
      return {
        id:s.id,title:s.title,date:s.scheduled_date,className:classById.get(s.class_id)?.name??"Unknown class",
        subjectName:subjectById.get(s.subject_id)?.name??"Unknown subject",teacherName:teacherById.get(s.teacher_id??"")?.full_name??"Unassigned",
  ...
  for(const r of readiness){
    ...
    if(!r.paper&&days<=2)dangers.push({severity:days<=1?"urgent":"high",label:"Paper missing",detail:`${r.title} is due ${r.date}.`,href:"/coordinator/papers"});
    if(r.approved&&!r.printed&&days<=1)dangers.push({severity:"critical",label:"Approved paper not printed",detail:`${r.title} is scheduled ${r.date}.`,href:"/clerk/papers"});
    if(p?.status==="draft"&&p.review_notes)dangers.push({severity:"high",label:"Paper needs correction",detail:`${r.title} has review feedback awaiting a revision.`,href:"/coordinator/papers"});
  }
  ...
  if(missingResults.length) dangers.push({severity:missingResults.length>=3?"critical":"high",label:"Results incomplete",detail:`${missingResults.length} completed test(s) still have missing student results.`,href:"/coordinator/results"});
  if(overdueTests.length) dangers.push({severity:overdueTests.length>=3?"critical":"urgent",label:"Tests overdue",detail:`${overdueTests.length} scheduled test(s) are past their date without completion.`,href:"/coordinator/schedule"});
  ...
  for(const st of students){
    ...
    const weakSubjects=Array.from(subjectGroups.entries()).sort((a,b)=>(b[1].fail/Math.max(b[1].total,1))-(a[1].fail/Math.max(a[1].total,1))).filter(([,g])=>g.fail>0).slice(0,3).map(([id])=>subjectById.get(id)?.name??"Unknown");
    const score=Math.min(100,Math.round(failed/Math.max(graded.length,1)*65+Math.max(0,60-average)*.8+(consecutive?25:0)));
    if(score>=35)studentRisks.push({id:st.id,name:st.name,rollNo:st.roll_no,className:classById.get(st.class_id)?.name??"Unknown class",score,failed,average,weakSubjects});
  }
  ...
  const topicHeatmap=Array.from(topicStats.entries())
    .map(([topicId,g])=>({topicId,name:topicById.get(topicId)?.name??"Unknown topic",subjectName:subjectById.get(scheduleById.get((results.find(r=>r.schedule_item_id&&scheduleById.get(r.schedule_item_id)?.topic_id===topicId)?.schedule_item_id??"")||"")?.subject_id??"")?.name??"Unknown subject",attempts:g.attempts,failRate:pct(g.failed,g.attempts),average:pct(g.marks,g.total)}))
  ...
  for(const s of schedules.filter(x=>x.status==="completed")){
    ...
    if(distinct.size===1)anomalies.push({severity:"high",label:"Identical marks",detail:`${s.title}: all ${rows.length} graded students have the same mark.`,href:"/coordinator/results"});
    else if(passRate>=95)anomalies.push({severity:"warning",label:"Unusually high pass rate",detail:`${s.title}: ${passRate}% passed (${rows.length} graded).`,href:"/coordinator/results"});
    else if(passRate<=10)anomalies.push({severity:"high",label:"Unusually low pass rate",detail:`${s.title}: only ${passRate}% passed (${rows.length} graded).`,href:"/coordinator/results"});
  }
  ...
  const interventionEffectiveness:InterventionEffectivenessRow[]=interventions
    ...
    .map(i=>{
      ...
      return {id:i.id,studentName:students.find(s=>s.id===i.student_id)?.name??"Unknown student",action:i.action,followUpDate:i.follow_up_date??"",beforeAverage:before,afterAverage:after,improvement,outcome:i.outcome,status:i.status};
  ...
  const dailyBrief:DailyBrief={
    ...
    headline:dangers.length===0?"No active academic danger signals today.":`${dangers.length} active academic issue${dangers.length===1?"":"s"} need attention.`
  };

  const healthMetrics:HealthMetric[]=[
    {label:"Exam readiness",score:upcomingReadiness,detail:`${readiness.length} exam(s) in the next 7 days`},
    {label:"Teacher compliance",score:teacherScore,detail:"Paper, test, and result completion"},
    {label:"Result completion",score:resultCompletion,detail:`${completedSchedules.length} completed test(s)`},
    {label:"Student performance",score:performanceScore,detail:graded.length?`${graded.length} graded attempts`:"No graded data yet"},
    {label:"Syllabus progress",score:syllabusScore,detail:`${topicsCovered.size} of ${topicCountRes.count??0} topics covered`}
  ];
