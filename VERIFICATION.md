...
## Real bugs found live (owner clicking through the actual deployed app)
This is exactly why "reviewed correct" and "verified" are different things - the app had never actually been clicked through end-to-end with real keyboard input until now.

- [x] **Root `/` was a dead stub.** `app/page.tsx` since Phase 0 was just `<main>School OS</main>` - no link to login, looked broken. Fixed to a smart redirect: signed-in users go straight to `/owner` or `/teacher`, everyone else to `/login`.
- [x] **Every dialog in the entire app silently broke typing after one character.** Root cause: `Dialog`'s focus-on-open `useEffect` depended on `handleKeyDown`, which depended on `onClose` - and every call site passes a fresh inline `() => setOpen(false)` on every render. Typing a character -> parent re-renders -> new `onClose` reference -> effect re-fires -> `setTimeout` steals focus back to the dialog wrapper. Fixed by moving `onClose` into a ref and scoping the effect to `[open]` only. This affected every single dialog built by every agent this session (syllabus, students, teachers, papers, schedule) - none of us caught it because none of us had tested actual live typing in a real browser until the owner did.
- [x] **No path for teachers without email.** The original invite flow required a real email address (Supabase's `inviteUserByEmail`), but most teachers at this school don't have one. Rebuilt as owner-sets-username-and-password-directly: `lib/auth/username.ts` derives a deterministic synthetic email (`<username>@teacher.schoolos.local`, never delivered anywhere) purely to satisfy Supabase Auth's internal email requirement; `profiles.username` (case-insensitive unique index, migration `0012`) is the real human-facing login identifier. Login now accepts username OR email. Owner can also reset a teacher's password directly (no email-based recovery exists for these accounts).
- [x] **Teacher accounts had no way to become useful.** `teacher_subjects` (the table every single RLS policy in the app reads through) had zero UI anywhere to create a row - an owner could create a teacher account but had no way to ever let them see anything. Added an "Assign subjects" dialog per teacher (class -> subject picker, add/remove) plus a role selector on account creation (teacher or owner - the schema/triggers already supported multiple owners correctly, just needed a UI to use it).

All of the above: typecheck/lint/build gates green, migration `0012` applied and advisor-checked (no new findings), unique-index correctness verified against its actual index definition (case-insensitive collision-blocking is standard, well-established Postgres semantics for this exact index shape - a second real auth user would be needed to additionally prove it empirically).

## First real teacher account — live end-to-end
