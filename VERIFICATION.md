...
1. **Live RLS proof with real principal/academic_coordinator/clerk accounts is still pending** - requires the owner to create one test account per new role (same flow already used for the two real teacher accounts). Once created, the exact per-role SQL assertions from the plan's Verification section can be run.
2. Manual click-through verification (dev server / real login) of all five dashboards has not been done - only typecheck/lint/build and static code review.
